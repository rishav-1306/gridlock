...
print("[3] Computing spatial clusters...")
...
for gh in all_ghs:
    ...
    nbrs  = [train_gh_list[i] for i in idxs if train_gh_list[i] != gh][:3]
...
cluster_stats = train[train['day']==48].groupby(
    ['spatial_cluster','time_mins']
...
def add_lag_features(df, pivot48, pivot49e):
    df = df.copy()
    for lag in LAGS:
        ...
        df[f'd48_lag{lag}']  = v48
        df[f'd49e_lag{lag}'] = v49e
    spill = []
...
def build_features(df):
    ...
    df = add_lag_features(df, pivot48, pivot49e)
    ...
    df['lag49e_std']   = df[lag49e_cols].std(axis=1)
...
print("[8] Training Neural Network (MLP Regressor)...")
try:
    import torch
    ...
    class TrafficMLP(nn.Module):
        def __init__(self, n_features, hidden=256, n_blocks=4, dropout=0.2):
            super().__init__()
    ...
    HAS_TORCH = False
    print("    PyTorch not found — skipping NN. Install with: pip install torch")
    oof_nn  = None
    pred_nn = None
...
for i, (tr_idx, val_idx) in enumerate(folds):
    ...
    m.fit(X_cat[tr_idx], y[tr_idx], eval_set=(X_cat[val_idx], y[val_idx]), use_best_model=True)
...
print("\n" + "="*55)
print("  PIPELINE RESULTS (GroupKFold - no data leakage)")
print("="*55)
print(f"  LightGBM OOF:     {lgb_score:.4f}")
print(f"  XGBoost  OOF:     {xgb_score:.4f}")
print(f"  CatBoost OOF:     {cat_score:.4f}")
