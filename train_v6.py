"""
train_v6.py  —  Gridlock Traffic Demand Prediction (Version 6)
===========================================================
The definitive, high-generalization, 100% leak-free stacked ensemble.

KEY BREAKTHROUGHS & FEATURES IN v6:
  1. LEAVE-ONE-OUT DAY 49 MORNING ANCHORS:
     Calculates aggregated stats (mean, std, median, max, min) of the 9 early 
     morning timestamps (0 to 120 mins) for each geohash on Day 49.
     - For training rows (time_mins = t), the stats are computed by strictly 
       excluding the value at t to prevent target leakage.
     - For test rows (time_mins > 120), the stats are computed using all 9 early 
       timestamps.

  2. DAY-OVER-DAY SCALING MULTIPLIER (d48_lag0_scaled):
     We compute a highly localized scaling factor:
       day_scale_factor = d49_morning_mean / (gh_mean48_early + 1e-6)
     Where gh_mean48_early is the average early morning demand on Day 48.
     By scaling Day 48's same-time demand:
       d48_lag0_scaled = d48_lag0 * day_scale_factor
     We provide the models with an extremely robust, leak-free daytime baseline estimate!

  3. LEAVE-ONE-OUT SPATIAL MORNING ANCHORS (d49_morning_spatial_mean):
     For each geohash, we compute the leave-one-out early morning demand of its 
     5 nearest spatial neighbors on Day 49, giving powerful spatial-temporal context.
"""
...
# Day 48 early morning stats (0 to 120 mins) per geohash
d48_early = day48[day48['time_mins'] <= 120]
gh_early_stats = d48_early.groupby('geohash')['demand'].mean().reset_index()
gh_early_stats.columns = ['geohash', 'gh_mean48_early']

# Per-(geohash, hour) stats (day48)
...
# ─────────────────────────────────────────────────────────────────────────────
print("[6] Engineering leak-free features...")
...
def build_features(df, is_train=True):
    ...
    # Day 48 trends
    df['d48_trend_1h'] = df['d48_lag0'] - df['d48_lag-60']
    df['d48_trend_2h'] = df['d48_lag0'] - df['d48_lag-120']
    ...
    df = df.merge(gh_hour_stats, left_on=['lookup_gh','hour'], right_on=['geohash','hour'], how='left', suffixes=('','_ghs'))
    if 'geohash_ghs' in df.columns: df.drop(columns=['geohash_ghs'], inplace=True)
    df = df.merge(gh_early_stats, left_on='lookup_gh', right_on='geohash', how='left', suffixes=('','_ges'))
    if 'geohash_ges' in df.columns: df.drop(columns=['geohash_ges'], inplace=True)
    ...
    # 🚀 LEAVE-ONE-OUT DAY 49 MORNING ANCHORS (100% Leak-Free)
    anchors = [0, 15, 30, 45, 60, 75, 90, 105, 120]
    morning_means = []
    morning_stds = []
    morning_medians = []
    morning_maxs = []
    morning_mins = []
    morning_spatial_means = []
    ...
    for gh, t in zip(df['lookup_gh'], df['time_mins']):
        ...
        for a in anchors:
            if is_train and a == t:
                continue
            if gh in pivot49e.index and a in pivot49e.columns:
                vals.append(pivot49e.loc[gh, a])
        
        if vals:
            morning_means.append(np.mean(vals))
            morning_stds.append(np.std(vals))
            morning_medians.append(np.median(vals))
            morning_maxs.append(np.max(vals))
            morning_mins.append(np.min(vals))
        else:
            morning_means.append(np.nan)
            morning_stds.append(np.nan)
            morning_medians.append(np.nan)
            morning_maxs.append(np.nan)
            morning_mins.append(np.nan)
            
        # LOO Spatial Morning Anchor
        nbrs = spatial_neighbors.get(gh, [])
        sp_vals = []
        for n in nbrs:
            for a in anchors:
                if is_train and a == t:
                    continue
                if n in pivot49e.index and a in pivot49e.columns:
                    sp_vals.append(pivot49e.loc[n, a])
        morning_spatial_means.append(np.mean(sp_vals) if sp_vals else np.nan)
            
    df['d49_morning_mean']   = morning_means
    df['d49_morning_std']    = morning_stds
    df['d49_morning_median'] = morning_medians
    df['d49_morning_max']    = morning_maxs
    df['d49_morning_min']    = morning_mins
    df['d49_morning_spatial_mean'] = morning_spatial_means

    # 🚀 DAY-OVER-DAY SCALING baselines (Highly optimized!)
    df['day_scale_factor'] = df['d49_morning_mean'] / (df['gh_mean48_early'] + 1e-6)
    df['d48_lag0_scaled']  = df['d48_lag0'] * df['day_scale_factor']
...
# Only train on Day 49 early rows (leak-free)
train_fe_d49 = train_fe[train_fe['day'] == 49].reset_index(drop=True)
print(f"    Training set limited to Day 49 early rows: {train_fe_d49.shape[0]} rows")

X      = train_fe_d49[FEATURES].values.astype(np.float32)
y      = train_fe_d49['demand'].values.astype(np.float32)
X_test = test_fe[FEATURES].values.astype(np.float32)

# ─────────────────────────────────────────────────────────────────────────────
print("[7] Setting up GroupKFold on geohash groups...")
N_FOLDS = 5
groups  = train_fe_d49['geohash_enc'].values
...
for i, (tr_idx, val_idx) in enumerate(folds):
    ...
    print(f"    Fold {i+1}: {max(0,100*r2_score(y[val_idx],oof_lgb[val_idx])):.4f}  (iter: {m.best_iteration_})")
...
cat_params = {
    ...
    'early_stopping_rounds': 150, 'thread_count': -1,
    'l2_leaf_reg': 3.0,
}
...
print("\n" + "="*55)
print("  PIPELINE RESULTS  (v6 — 100% LEAK-FREE)")
print("="*55)
print(f"  LightGBM OOF:     {lgb_score:.4f}")
print(f"  XGBoost  OOF:     {xgb_score:.4f}")
print(f"  CatBoost OOF:     {cat_score:.4f}")
...
print(f"  Stacked  OOF:     {meta_score:.4f}  <- final GroupKFold")
print("="*55)
...
print(f"\nSaved submission_v6.csv  ({submission.shape[0]} rows)")
